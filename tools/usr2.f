...
* Last Modified : 2010年04月20日 (週二) 10時08分14秒
...
* ========================================================================
      ...
      do I0=1,total_usr_moment
        usr_moment(I0)=0.D0
      enddo
C      do I0=1,usr_3_moment_max
C        usr_3_moment(I0)=0.D0
C      enddo
C      do I0=1,usr_4_moment_max
C        usr_4_moment(I0)=0.D0
C      enddo
C      do I0=1,usr_4_moment_ex_max
C        usr_4_moment_ex(I0)=0.D0
C      enddo
      return
      end
...
C===========================================
     ...
     &+(x(J1-1)-ctd_y)**2+(x(J1)-ctd_z)**2)   !record dist_ct
      ...
      enddo
C=========We don't accept J0=fct_id. Total n=atom_num-1.========
      J2=fct_id*3
      do J0=1,fct_id-1
        J1=J0*3
     ...
     &(x(J1)-x(J2))**2)      !record dist_fct
      ...
      do J0=fct_id+1,atom_num
        J1=J0*3
        dist_fct(J0)=dsqrt((x(J1-2)-x(J2-2))**2+(x(J1-1)-x(J2-1))**2+
     &(x(J1)-x(J2))**2)      !record dist_fct to index(J0-1) total number atom_num-1
        if(dist_fct(J0).gt.dist_fct_to_ftf)then
          dist_fct_to_ftf=dist_fct(J0)
          ftf_id=J0         !determine ftf atom
        endif
      enddo
C=========We don't accept J0=cts_id. Total n=atom_num-1.
      J3=cst_id*3
      do J0=1,cst_id-1
        J1=J0*3
     ...
     &(x(J1-1)-x(J3-1))**2+(x(J1)-x(J3))**2)
      enddo
C=========We don't accept J0=ftf_id. Total n=atom_num-1.
      J4=ftf_id*3
      do J0=1,ftf_id-1    
        J1=J0*3 
...
C moment, and finally 1~20 with adev data
      ...
      call moment(dist_ctd,atom_num,ave,adev,sdev,var,skew,curt)
...
C      usr_4_moment_ex(5)=curt
      call moment(dist_cst,atom_num-1,ave,adev,sdev,var,skew,curt)
...
C      usr_4_moment_ex(10)=curt
      call moment(dist_fct,atom_num-1,ave,adev,sdev,var,skew,curt)
...
C      usr_4_moment_ex(15)=curt
      call moment(dist_ftf,atom_num-1,ave,adev,sdev,var,skew,curt)
