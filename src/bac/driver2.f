...
c     variables).
      ...
      integer          nmax, mmax,atoms,natoms_a,ndim
...
c     Declare a few additional variables for the sample problem.
      ...
      real*8   X4(atoms*3),xf(atoms*3),g1(atoms*3)
      integer          i
      common /atoms /atoms,natoms_a,ndim  
...
c         t1=x(2)-x(1)**2
c         g(1)=2.d0*(x(1)-1.d0)-1.6d1*x(1)*t1
c         do 22 i=2,n-1
c            t2=t1
c            t1=x(i+1)-x(i)**2
c            g(i)=8.d0*t2-1.6d1*x(i)*t1
c  22     continue
c         g(n)=8.d0*t1
            call agrad(x,g)
c       do  i=1,n
...
c         if (isave(34) .ge. 99)
c     +      task='STOP: TOTAL NO. of f AND g EVALUATIONS EXCEEDS LIMIT'
...
c         write (6,'(2(a,i5,4x),a,1p,d12.5,4x,a,1p,d12.5)') 'Iterate'
c     +      ,isave(30),'nfg =',isave(34),'f =',f,'|proj g| =',dsave(13)
