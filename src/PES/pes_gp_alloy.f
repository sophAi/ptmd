*========================================================================
* File Name : pes_gp_alloy.f 
* Copyright (C) 2008-2011 Po-Jen Hsu <xanadu8850@pchome.com.tw>
* Creation Date : 19-04-2010
* Last Modified : Fri 27 Jan 2012 12:31:09 PM CST
...
C   read p1,q1,epsilon1,zeta1,rzero1
      ...
      atom_num_a=0
      do I1=1,atom_num
        if(atom_name(I1).eq.atom_name_a)then
          atom_num_a=atom_num_a+1
        else
          atom_name_b=atom_name(I1)
        endif
      enddo
      atom_num_b=atom_num-atom_num_a
     ...
     &(read_real8(I0),I0=1,total_num)
      ...
      do J1=1,atom_num
        do J2=1,atom_num
          if(atom_name(J1).eq.atom_name_a.and.
     &atom_name(J2).eq.atom_name_a)then
            epsilon_ab(J1,J2)=epsilon1
            zeta_ab(J1,J2)=zeta1
            p_ab(J1,J2)=p1
            q_ab(J1,J2)=q1
            rzero_ab(J1,J2)=rzero1
            bond_cutoff(J1,J2)=rzero1*bond_cutoff_ratio
          else if(atom_name(J1).eq.atom_name_b.and.
     &atom_name(J2).eq.atom_name_b)then
            epsilon_ab(J1,J2)=epsilon2
            zeta_ab(J1,J2)=zeta2
            p_ab(J1,J2)=p2
            q_ab(J1,J2)=q2
            rzero_ab(J1,J2)=rzero2
            bond_cutoff(J1,J2)=rzero2*bond_cutoff_ratio
          else
            ...
            bond_cutoff(J1,J2)=rzero3*bond_cutoff_ratio
          endif
        enddo
      enddo
      rzero_max=rzero1
...
C      rcut_unit=3.D0*dble(atom_num)**(1.D0/3.D0)
      rcut_unit=2.3D0*dble(atom_num)**(1.D0/3.D0)
      rcut=rcut_unit*rzero_max
      moving_length=rzero_max
...
*************************************************************************
      ...
      real*8 pes_gupta_alloy_function,dist2
      real*8 ATT(ndim_max),REP(ndim_max),ATT_DUMMY,REP_DUMMY
      real*8 POT_REP,POT_ATT
     ...
     &x(J4-1))**2+(x(J3)-x(J4))**2)
            dist(J1,J2)=DSQRT(dist2)
            ATT_DUMMY=DEXP(2.D0*Q_AB(J1,J2)*
     &(1.D0-(dist(J1,J2)/RZERO_AB(J1,J2))))
     ...
     &*REP_DUMMY/(dist(J1,J2)*RZERO_AB(J1,J2))
            GRAD_ATTM(J1,J2)=(ZETA_AB(J1,J2)**2)*(-1.D0)*
     &Q_AB(J1,J2)*ATT_DUMMY/(dist(J1,J2)*RZERO_AB(J1,J2))
            if(dist(J1,J2).LE.RZERO_AB(J1,J2)*1.2D0)
     &bond_num(J1)=bond_num(J1)+1.D0
C===============Only for evaperation in BIMD===============
            if(dist(J1,J2).gt.rcut)then
              evap_num=evap_num+1
              GRAD_REPM(J1,J2)=
     &GRAD_REPM(J1,J2)+EPSILON_AB(J1,J2)*
     &(-P_AB(J1,J2)/RZERO_AB(J1,J2))*3.D0*(rcut/dist(J1,J2)-1.D0)    !Better be masked in other jobs
            endif
C================End=======================================
          ENDIF
        ENDDO
